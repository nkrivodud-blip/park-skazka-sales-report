...
test("server-renders the August sales report", async () => {
  ...
  assert.match(html, /Александр Воронин/);
  assert.match(html, /36,2<!-- -->%/);
  assert.match(html, /Методика конверсии/);
  assert.match(html, /Сырой pipeline/);
  assert.doesNotMatch(html, /Александр Поленко/);
  assert.match(html, /146 мероприятий/);
  assert.match(html, /NQL лиды по неделям/);
  assert.match(html, /17–23.08/);
  assert.match(html, /88,2%/);
  assert.doesNotMatch(html, /Очищенные лиды|Периметр выборки|Удалено дублей/);
  assert.match(html, /Утилизация площадок/);
  assert.match(html, /Айва-парк · все беседки/);
